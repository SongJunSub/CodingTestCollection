...
public class FailureRate {
    ...
    public int[] solution(int N, int[] stages) {
        ...
        for (int i = 1; i <= N; i++) {
            int failCount = 0;
            List<Integer> nextStages = new ArrayList<>();
            ...
            if (stages.length == 0) {
                ...
                failures[i - 1] = (double) failCount / stages.length;
            }

            stages = new int[nextStages.size()];

            for (int j = 0; j < nextStages.size(); j++) {
                stages[j] = nextStages.get(j);
            }
        }
        ...
        failuresList.sort((a, b) -> {
            if (failures[a - 1] == failures[b - 1]) {
                return Integer.compare(a, b);
            }

            return Double.compare(failures[a - 1], failures[b - 1]);
        });

        for (int i = 0; i < N; i++) {
            answer[i] = failuresList.get(i);
        }
