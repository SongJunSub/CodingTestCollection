...
public class DataAnalysis {
    ...
    public int[][] solution(int[][] data, String ext, int val_ext, String sort_by) {
        int[][] answer = {};
        int[][] exceptValExt = {};
        int exceptValExtLength = 0;
        int exceptionValExtIndex = 0;
        ...
        for (int i = 0; i < data.length; i++) {
            ...
            exceptValExt = new int[exceptValExtLength][data[0].length];
        }

        for (int i = 0; i < data.length; i++) {
            for (int j = 0; j < data[i].length; j++) {
                if (exceptionValExtIndex <= exceptValExtLength - 1 && data[i][1] < val_ext) {
                    exceptValExt[exceptionValExtIndex][j] = data[i][j];
                }
            }

            if (data[i][1] < val_ext) {
                exceptionValExtIndex++;
            }
        }

        Arrays.sort(exceptValExt, (a, b) -> Integer.compare(a[3], b[3]));
